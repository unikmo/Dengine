...
const STATUS_LABELS:Record<string,string>={not_started:'Not started',in_progress:'In progress',blocked:'Blocked',awaiting_approval:'Awaiting approval',done:'Done'}
...
export default function WorkspaceClient({workspaceId}:{workspaceId:string}){
  ...
  return <main className="min-h-screen bg-[#f5f2ea] pb-20">
    <section className="border-b border-black/[.055] bg-[#15233f] text-white"><div className="shell py-9">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#efcd6d]">Live execution workspace · {data.role}</p><h1 className="display mt-3 text-4xl font-black sm:text-5xl">{data.workspace.name}</h1><p className="mt-3 text-sm text-white/55">Changes sync every 10 seconds for collaborators. Task updates recalculate readiness immediately.</p></div>
...
function RunOfShow({workspaceId,items,canWrite,reload,setMessage}:{workspaceId:string,items:any[],canWrite:boolean,reload:()=>Promise<void>,setMessage:(s:string)=>void}){
  ...
  async function save(payload:any){const res=await fetch(`/api/workspaces/${workspaceId}/run-of-show`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const out=await res.json().catch(()=>({}));if(!res.ok){setMessage(out.error||'Run of Show update failed.');return}setForm({startTime:'',durationMinutes:15,cue:'',owner:'',location:'',technicalCue:'',contingency:'',notes:'',status:'planned'});await reload()}
  async function remove(id:string){await fetch(`/api/workspaces/${workspaceId}/run-of-show`,{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});await reload()}
  return <section className="mt-6"><div className="flex items-end justify-between gap-4"><div><p className="eyebrow">Event day</p><h2 className="mt-2 text-2xl font-black text-[#23324a]">Run of Show</h2><p className="mt-2 max-w-2xl text-sm text-[#687386]">Exact cues, owners, locations, handoffs and contingencies for live execution.</p></div><GitBranch className="text-[#9a7b31]"/></div>
    ...
    <div className="panel mt-5 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-[#fbfaf7] text-[10px] uppercase tracking-[.1em] text-[#8a93a2]"><tr><th className="p-4">Time</th><th className="p-4">Cue</th><th className="p-4">Owner</th><th className="p-4">Location</th><th className="p-4">Status</th><th className="p-4">Contingency</th><th className="p-4"></th></tr></thead><tbody>{items.map((r:any)=><tr key={r.id} className="border-t"><td className="p-4 font-black">{r.start_time?.slice(0,5)||'TBD'}</td><td className="p-4"><p className="font-black text-[#23324a]">{r.cue}</p>{r.technical_cue&&<p className="mt-1 text-xs text-[#7a8595]">Cue: {r.technical_cue}</p>}</td><td className="p-4">{r.owner_label||'—'}</td><td className="p-4">{r.location||'—'}</td><td className="p-4"><span className="font-black">{r.status}</span></td><td className="p-4 text-xs text-[#687386]">{r.contingency||'—'}</td><td className="p-4">{canWrite&&<button onClick={()=>remove(r.id)} className="text-xs font-black text-red-700">Remove</button>}</td></tr>)}</tbody></table></div>
  </section>
}
