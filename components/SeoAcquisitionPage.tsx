...
export default function SeoAcquisitionPage({slug,eyebrow,title,lead,intro,workstreams,steps,outputs,pitfalls,cta='Build my execution plan',faqs=[],related=[]}:SeoAcquisitionPageProps){
  ...
  const breadcrumbSchema=slug?{'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'RunYourEvent',item:'https://runyourevent.com/'},{'@type':'ListItem',position:2,name:'Resources',item:'https://runyourevent.com/resources'},{'@type':'ListItem',position:3,name:eyebrow,item:`https://runyourevent.com/${slug}`}]}:null
  return <main className="bg-[#fbfaf7]">
    ...
    {related.length>0&&<section className="border-b border-black/[.055] bg-[#f5f2ea] py-16"><div className="shell"><p className="eyebrow">Related execution guides</p><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{related.map(item=><a key={item.href} href={item.href} className="rounded-[22px] border border-black/[.055] bg-white p-5 font-black leading-6 text-[#23324a] transition hover:-translate-y-0.5 hover:border-[#c9aa57]">{item.title}<ArrowRight className="mt-4 text-[#9a7b31]" size={15}/></a>)}</div></div></section>}
