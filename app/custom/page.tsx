...
import { ChevronRight, Clock, Search, Users, Zap } from 'lucide-react'

import { supabase } from '@/lib/supabase'
import type { BudgetLevel, Event, GeneratedTask } from '@/types'
...
export default function CustomEventPage() {
  ...
  async function generate() {
    ...
    await supabase.from('blueprints').insert({
      ...
      is_volunteer_driven: isVolunteer,
      tasks: [],
    })
  ...
  const tasksByLayer = layers.reduce((acc, l) => {
    ...
    return acc
  }, {} as Record<(typeof layers)[number], GeneratedTask[]>)
    ...
    <main className="max-w-3xl mx-auto px-6 py-10">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-gray-400">
                      {ev.category} · {ev.has_tasks ? 'Full blueprint ready' : 'Profile available'}
                    </div>
                  </div>
            ...
            {BUDGET_OPTIONS.map((opt) => (
              <button
                ...
                className={`p-3 rounded-xl text-center transition-all border-2 ${
                  budgetLevel === opt.level ? 'border-navy bg-navy text-white' : 'border-gray-100 bg-gray-50 text-gray-700 hover:border-gray-200'
                }`}
              >
        ...
        <div className="flex flex-wrap gap-3">
          {(
            [
              ['Volunteer-driven (15-min tasks)', isVolunteer, setIsVolunteer, '🙌'],
              ['First time running this event', isFirstTime, setIsFirstTime, '🆕'],
              ['Outdoor event', isOutdoor, setIsOutdoor, '🌤️'],
            ] as const
          ).map(([label, value, setter, icon]) => (
            <label
              key={label}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 cursor-pointer transition-all text-sm font-medium ${
                value ? 'border-navy bg-navy/5 text-navy' : 'border-gray-100 bg-gray-50 text-gray-600'
              }`}
            >
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl font-bold text-base transition-all flex items-center justify-center gap-2 ${
            eventName.trim() && !generating ? 'bg-gold text-navy hover:bg-yellow-300 shadow-sm' : 'bg-gray-100 text-gray-400 cursor-not-allowed'
          }`}
        >
          {generating ? (
            <>
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <p className="text-gray-500">
                {tasks.length} tasks · {Math.round((tasks.reduce((s, t) => s + t.time_minutes, 0) / 60) * 10) / 10}h total
                {isVolunteer && ' · All ≤15 min'}
              </p>
            </div>

            <button onClick={() => window.print()} className="bg-navy text-white px-4 py-2 rounded-lg text-sm font-semibold">
              Print
            </button>
          </div>
                  ...
                  <div className="space-y-2">
                    {lt.map((task, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          onClick={() => isVolunteer && !claimed && claimTask(idx)}
                          className={`bg-white rounded-xl border p-4 transition-all ${
                            isVolunteer && !claimed ? 'cursor-pointer hover:border-navy/20 hover:shadow-sm' : ''
                          } ${claimed ? 'border-green-200 bg-green-50' : 'border-gray-100'}`}
                        >
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                              ...
                              {claimed ? (
                                <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Done {claimed}</span>
                              ) : (
                                isVolunteer && <span className="text-xs text-gray-400">tap to claim</span>
                              )}
                            </div>
                          </div>
                        </div>
