...
export default function CustomEventPage() {
  ...
  function pickSuggestion(ev: Event) {
    setEventName(ev.name); setMatchedEvent(ev); setFoundInDB(true); setSuggestions([]); setShowSug(false)
  }
  async function generate() {
    if (!eventName.trim()) return
    setGenerating(true); setTasks([]); setClaimedBy({})
    ...
    try {
      const fakeEvent = { id: 'custom', name: eventName, category: 'Custom', subcategory: 'Custom', scale: guests < 50 ? 'Intimate' : guests < 500 ? 'Medium' : guests < 5000 ? 'Large' : 'Mega', blueprint: 'General', luxury_base: budgetLevel, complexity: 3, planning_weeks: 4, description: `${eventName} for approximately ${guests} guests.`, key_dimensions: ['logistics','catering','coordination'], primary_cost: 'Venue and catering', key_risks: [], intake_questions: [], has_tasks: false }
  ...
  function claimTask(idx: number) { setClaimedBy(p => ({ ...p, [idx]: claimName.trim() || 'Anonymous' })) }
  const tasksByLayer = layers.reduce((acc, l) => { acc[l] = tasks.filter(t => t.layer === l); return acc }, {} as Record<string, GeneratedTask[]>)
  return (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
        <div className="relative">
          ...
          <div className="relative">
            ...
            <input value={eventName} onChange={e => { setEventName(e.target.value); setFoundInDB(false); setMatchedEvent(null) }} placeholder="e.g. Charity Gala, School Fair, Team Building Day..." className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy text-base"/>
          </div>
        ...
        <div>
          ...
          <input type="number" min="1" value={guestCount} onChange={e => setGuestCount(e.target.value)} placeholder="e.g. 200" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy text-base"/>
        </div>
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-3">Budget approach</label>
          <div className="grid grid-cols-5 gap-2">
            {BUDGET_OPTIONS.map(opt => (
              <button key={opt.level} onClick={() => setBudgetLevel(opt.level as BudgetLevel)} className={`p-3 rounded-xl text-center transition-all border-2 ${budgetLevel === opt.level ? 'border-navy bg-navy text-white' : 'border-gray-100 bg-gray-50 text-gray-700 hover:border-gray-200'}`}>
        ...
        <div className="mt-8">
          ...
          <div className="space-y-6">
            {layers.map(layer => {
              ...
              return (
                <div key={layer}>
                  <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-lg mb-3 text-sm font-bold ${colors.bg} ${colors.text} border ${colors.border}`}>{layer} <span className="font-normal opacity-60">({lt.length})</span></div>
                  <div className="space-y-2">
                    {lt.map((task, i) => {
                      const idx = tasks.indexOf(task)
                      const claimed = claimedBy[idx]
                      return (
                        <div key={i} onClick={() => isVolunteer && !claimed && claimTask(idx)} className={`bg-white rounded-xl border p-4 transition-all ${isVolunteer && !claimed ? 'cursor-pointer hover:border-navy/20 hover:shadow-sm' : ''} ${claimed ? 'border-green-200 bg-green-50' : 'border-gray-100'}`}>
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-gray-900 text-sm">{task.title}</p>
cat > app/custom/page.tsx << 'ENDOFFILE'
'use client'
import { useState, useEffect } from 'react'
import { Zap, Clock, Users, Search, ChevronRight } from 'lucide-react'
import { supabase } from '@/lib/supabase'
